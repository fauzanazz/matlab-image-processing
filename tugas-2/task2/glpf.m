function [output, H, spectrum] = glpf(f, D0, usePadding)
% GLPF - Gaussian Low-Pass Filter dalam ranah frekuensi
%
% Syntax:
%   [output, H, spectrum] = glpf(f, D0, usePadding)
%
% Input:
...
%
% Formula GLPF (Slide 26):
%   H(u,v) = exp(-D^2(u,v) / (2*D0^2))
%   dimana D(u,v) = sqrt((u - M/2)^2 + (v - N/2)^2)
%
% Catatan Slide 25: GLPF tidak menimbulkan ringing effect
% karena tidak ada transisi tajam seperti ILPF
    ...
    for ch = 1:channels
        ...
        % GLPF: H(u,v) = exp(-D^2/(2*D0^2)) (Slide 26, 30-31)
        H = exp(-(D.^2)./(2*(D0^2)));
        H = fftshift(H);
        G = H .* F_shifted;
